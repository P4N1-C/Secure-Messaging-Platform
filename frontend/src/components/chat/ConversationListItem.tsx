  ...
  return (
    <div 
      ...
      className={`flex items-center cursor-pointer ${
        isCollapsed 
          ? `p-2 mx-auto w-[60px] h-[60px] justify-center rounded-2xl mb-2 transition-colors ${isSelected ? 'bg-gray-200' : 'hover:bg-gray-200'}` 
          : `p-3 mx-2 rounded-lg mb-1 transition-colors ${isSelected ? 'bg-gray-200' : 'hover:bg-gray-200'}`
      }`}
    >
      ...
      {!isCollapsed && (
        <div className="ml-3 flex-1 overflow-hidden">
          <div className="flex justify-between items-baseline">
            <span className="text-[15px] font-semibold truncate text-gray-900">{name}</span>
            {last_message && (
              <span className="text-xs text-gray-500 shrink-0 ml-2">{formatTime(last_message.created_at)}</span>
            )}
          </div>
