...
export function MemberRow({ member, currentUser, isAdmin, onRemove, onOpenUserInfo }: MemberRowProps) {
  ...
  return (
    <div className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg transition-colors group">
      ...
      >
        <div className="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400 font-medium shrink-0 overflow-hidden">
      ...
      {isAdmin && !isMe && member.role !== 'admin' && (
        <button 
          ...
          className="p-1.5 text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg opacity-0 group-hover:opacity-100 transition-all shrink-0"
          title="Remove Member"
        >
